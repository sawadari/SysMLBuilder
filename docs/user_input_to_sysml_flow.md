...
`requirements_*.md` は人が読む要求仕様書、`case.yaml` はそのケースだけで使う変換ルールと View 定義です。

## 入力 Markdown はどう書くのがよいか

おすすめは、次の 2 層に分けることです。

- Markdown
  人が読んでレビューできる背景、目的、対象範囲、要求
- `case.yaml`
  package 名、view 名、要素名、描画方針などのローカル rule/profile

標準フォーマットの全体像は
[standard_requirement_spec_format.md](standard_requirement_spec_format.md)
にまとめています。

要求の行は、今は次の書式を標準として扱います。

- `- CASE-ID: ...`
- `- [CASE-ID] ...`
- `1. [CASE-ID] ...`

逆に、Markdown の中へ大きな YAML や SysML 全文を埋め込む書き方は、サンプル用途では推奨しません。

canonical `.sysml` では、標準として次の View セットを出します。
