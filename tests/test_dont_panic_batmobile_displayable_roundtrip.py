...
CASE_DIR = ROOT / "example" / "dont_panic_batmobile_displayable"


class DontPanicBatmobileDisplayableRoundtripTest(unittest.TestCase):
    def test_markdown_roundtrips_to_expected_sysml(self) -> None:
        source = CASE_DIR / "input" / "batmobile_displayable_views_requirements.md"
        expected = (CASE_DIR / "output" / "dont_panic_batmobile_displayable_canonical.sysml").read_text(encoding="utf-8")

        result = transform_markdown(source)

        self.assertEqual(result.case_id, "dont_panic_batmobile_displayable")
        ...
        self.assertIn("filter DS_Functions::IsNonStandardLibraryElement();", result.canonical)
        self.assertIn("expose Dont_Panic_Batmobile::batmobileDesignV23;", result.canonical)
