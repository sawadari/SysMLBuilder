...
def run_suite(root: Path, suite_dir: Path | None = None) -> int:
    cases_root = suite_dir or (root / "testdata" / "gfse_derived")
    ...
    for case in manifest["cases"]:
        ...
        result = transform_markdown(cases_root / f"{case_id}_requirements.md")
        ...
        if not _compare_yaml(actual_contracts, cases_root / f"{case_id}_expected_contracts.yaml"):
        ...
        if result.canonical:
            ...
            if _normalized_text(actual_canonical.read_text(encoding="utf-8")) != _normalized_text((cases_root / f"{case_id}_expected_canonical.sysml").read_text(encoding="utf-8")):
        ...
        if result.overlay:
            ...
            if _normalized_text(actual_overlay.read_text(encoding="utf-8")) != _normalized_text((cases_root / f"{case_id}_expected_review_overlay.sysml").read_text(encoding="utf-8")):
        ...
        if result.projection_manifest:
            ...
            if not _compare_yaml(actual_projection, cases_root / f"{case_id}_expected_projection_manifest.yaml"):
